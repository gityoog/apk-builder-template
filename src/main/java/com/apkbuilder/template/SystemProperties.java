...
public class SystemProperties {
  ...
  public static Boolean set(String key, String value) {
    try {
      return (Boolean) SP.getMethod("set", String.class, String.class).invoke(null, key, value);
    } catch (Exception e) {
      return false;
    }
  }
   ...
   * '0', 'false' or
   * 'off' are considered false. Values 'y', 'yes', '1', 'true' or 'on' are
   * considered true. (case
   * sensitive). If the key does not exist, or has any other value, then the
   * default result is
   * returned.
   *
   * @param key
